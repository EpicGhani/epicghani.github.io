...
const ct = {
     ...
     */
    version: '3.2.0',
    meta: [{"name":"Combi Stack Blocks","author":"Panoptik Digitals","site":"CombiStackBlocks","version":"1.0.0"}][0],
    get width() {
...
ct.rooms.templates['Main'] = {
    ...
    onCreate() {
        /* room Main — core_OnRoomStart (On room start event) */
{
ct.transition.circleIn(750, 0x8069BF);
}
...
{
///
/// THIS EVENT IS FOR BLOCK SPAWNING AND COLOR RANDOMIZER
///
...
if(this.countdown <= 0)
{
    if(!this.dropping)
    {
        this.timer2 = this.dropTime;
        this.dropping = true;
    }

    if(this.selectedBlocks[2] == 0 && this.grid[0][2] == 0 && !this.rearranging)
    {
        var xPos = 704;
        ...
        {
            do {
                this.selectedBlocks[0] = Math.floor((Math.random() * 3) + 1);
                this.selectedBlocks[1] = Math.floor((Math.random() * 3) + 1);
                this.selectedBlocks[2] = Math.floor((Math.random() * 3) + 1);
            ...
            for (let i = 0; i < this.selectedBlocks.length; i++)
            {
                ...
                var blockCopy = ct.templates.copy(block, xPos, (384) - (256*(i+1)));
                blockCopy.scale.x = blockCopy.scale.y = 0.16;
                this.activeBlocks.push(blockCopy);
                this.activeBlocks.shift();
            }
        }
    } else if(this.grid[0][2] != 0)
        this.gameOver = true;

    if(this.gameOver && this.result == null)
    {
       this.result = ct.rooms.append('Game over');
        // if (!paused) ct.sound.pause('bgm')
        if (!paused) ct.sound.spawn('gameover')
...
{
    this.countdown--;
    this.countdownLabel.x = 790;
    this.countdownLabel.y = 900;
    this.countdownLabel.text = this.countdown;
    if (!paused) ct.sound.spawn('Sound_kFw56H')
}
else
{
    this.countdown--;
    this.countdownLabel.text = '';
    if (!paused) ct.sound.spawn('sweep_notifs')
    this.timer1 = 0;
}
...
this.timer2 = this.dropTime;
if(this.activeBlocks[0]!=null && !this.dropped && !this.gameOver)
{
    var matchFound = false;
    var matchType = 0; 

    var headCopy = this.activeBlocks[0];
    var iY = (headCopy.y+128)/256 - 1;
    var iX = (headCopy.x+64)/256-1;


    if(headCopy.y < 2176 && (iY) < 9 && this.grid[iY+1][iX] == 0)
    {
        for(let i = 0; i<this.activeBlocks.length;i++)
        {
            var copyRef = this.activeBlocks[i];
            copyRef.y+=256;
        }
    }
    else if(iY >= 0)
    {
        for(let i = 0; i<this.activeBlocks.length;i++)
        {
            if(iY-i >= 0)
            {
                this.grid[iY-i][iX] = this.selectedBlocks[i];
                this.gridBlocks[iY-i][iX] = this.activeBlocks[0];
                this.activeBlocks.push(null);
                this.selectedBlocks[i] = 0;
                this.activeBlocks.shift();
                console.log(this.selectedBlocks[i])
            }
            else
            {
                // console.log('hahaha')
...
/* room Game — core_OnActionPress (OnActionPress event) */

if (ct.actions['MoveX'].pressed) {
    let value = ct.actions['MoveX'].value;
    
if(this.activeBlocks[0] != null && ct.pixiApp.ticker.speed > 0)
{
    var headCopy = this.activeBlocks[0];
    var iY = (headCopy.y+128)/256-1;
    var iX = (headCopy.x+64)/256-1;
    ...
    if(iX+value > 0 || iX+value < 7)
    {
        if(this.grid[iY][iX+value]==0)
            adjustmentValue = 256*value;
    }
    else
    {
        adjustmentValue = 0;
    ...
    for(let i = 0;i < this.activeBlocks.length;i++)
    {
        this.activeBlocks[i].position.x += adjustmentValue;
    }
}
...
if (ct.actions['Switch'].pressed) {
    let value = ct.actions['Switch'].value;
...
if(this.activeBlocks[0]!=null && ct.pixiApp.ticker.speed > 0)
{
    ...
    {
        if(i==0)
            this.activeBlocks[i].y-=512;
        else
            this.activeBlocks[i].y+=256;
    }
    this.activeBlocks.push(this.activeBlocks.shift());
    this.selectedBlocks.push(this.selectedBlocks.shift());
}
...
if (ct.actions['Drop'].pressed) {
    let value = ct.actions['Drop'].value;
...
{    
    ...
    if (!paused) ct.sound.spawn('whoop')

    for(let i = iY; i < this.grid.length; i++) 
    {
        if(iX < this.grid[i].length) 
        {
            if(this.grid[i][iX] === 0) 
            {
                multiplier = i - iY;
    ...
    {
        var copyRef = this.activeBlocks[i];
        copyRef.y+= (256*multiplier);
    }
    for(let i = 0; i<this.activeBlocks.length;i++)
    {
        if(iY+(multiplier-i) >= 0)
        {
            this.grid[iY+(multiplier-i)][iX] = this.selectedBlocks[i];
            this.gridBlocks[iY+(multiplier-i)][iX] = this.activeBlocks[0];
            this.activeBlocks.push(null);
            this.selectedBlocks[i] = 0;
            this.activeBlocks.shift();
        }
...
if (ct.actions['MoveY'].pressed) {
    let value = ct.actions['MoveY'].value;
...
if(this.activeBlocks[0] != null && ct.pixiApp.ticker.speed > 0)
{
    if(value > 0)
    {
        ...
        for(let i = 0;i < this.activeBlocks.length;i++)
        {
            this.activeBlocks[i].position.y += adjustmentValue;
        }
    }
}
    ...
    onDraw() {
        /* room Game — core_OnDraw (On frame end event) */
{
if(ct.pointer.down[0] && ct.pixiApp.ticker.speed > 0 && !this.gameOver)
{
    
    if(this.startX == null)
        this.startX = ct.pointer.x;
    if(this.startY == null)
        this.startY = ct.pointer.y;
    ...
    if (Math.abs(this.lX) == 256) 
    {
        if(this.activeBlocks[0] != null)
        {
            this.moved = true;
            var headCopy = this.activeBlocks[0];
            var iY = (headCopy.y+128)/256-1;
            ...
            if(iX+(this.lX/256) > 0 || iX+(this.lX/256) < 7)
            {
                if(this.grid[iY][iX+(this.lX/256)]==0)
                for(let i = 0;i < this.activeBlocks.length;i++)
                {
                    this.activeBlocks[i].position.x += this.lX;
                }
            }
        }
        this.startX = null;
    ...
    if (this.lY == 256) 
    {
        if(this.activeBlocks[0] != null)
        {
            ...
            if(iY+(this.lY/256) > 0 && iY+(this.lY/256) < 9)
            {
                console.log(iY+(this.lY/256))
                if(this.grid[iY+(this.lY/256)][iX]==0)
                for(let i = 0;i < this.activeBlocks.length;i++)
                {
                    this.activeBlocks[i].position.y += this.lY;
                }
            }
        }
        this.startY = null;
...
{
    if(this.startX != null && Math.abs(this.lX) < 10 && !this.moved)
    {
        if(this.activeBlocks[0]!=null)
        {
            for (let i=0;i < this.activeBlocks.length;i++)
...
{
///
/// THIS EVENT IS FOR INITIALIZING VARIABLES AND OTHER STUFF
///
ct.transition.circleIn(750, 0x34b8fe);
ct.sound.spawn('bgm', {
     loop: true,
     volume: 0.5
});
...
//DEFINING GRID BLOCKS AND POPULATING
for(let y = 0;y<9;y++)
{
...
//SETUP LABEL FOR SCORING REFERENCE
...
//SETUP LABEL FOR GET READY SEQUENCE
...
//SETUP LABEL FOR LEVEL REFERENCE
...
//STARTUP TIMER FOR GET READY SEQUENCE
this.timer1 = 3;
...
ct.rooms.templates['Game paused'] = {
    ...
    width: 1600,
    height: 2400,
    objects: JSON.parse('[{"x":64,"y":0,"opacity":0.5,"tint":0,"scale":{"x":0.96,"y":0.97297297},"rotation":0,"exts":{},"customProperties":{},"template":"BG"},{"x":1730,"y":1830,"opacity":1,"tint":16777215,"scale":{"x":1,"y":1},"rotation":0,"exts":{},"customProperties":{},"template":"btnResume"},{"x":833,"y":1206.4694448,"opacity":1,"tint":16777215,"scale":{"x":2.18123667,"y":2.26438826},"rotation":0,"exts":{},"customProperties":{},"template":"NewGame"},{"x":833,"y":951,"opacity":1,"tint":16777215,"scale":{"x":2.18123667,"y":2.26438826},"rotation":0,"exts":{},"customProperties":{},"template":"btnContinue2"}]'),
    bgs: JSON.parse('[]'),
    tiles: JSON.parse('[]'),
    backgroundColor: '#000000',
    ...
    onCreate() {
        /* room Game paused — core_OnRoomStart (On room start event) */
{
this.alpha = 0;
this.cont;
...
ct.rooms.templates['Game over'] = {
    ...
    onCreate() {
        /* room Game over — core_OnRoomStart (On room start event) */
{
this.resultLabel = new PIXI.Text('Total Score\n'+score,ct.styles.get('MainLabel'))
this.addChild(this.resultLabel);
this.resultLabel.x = 820;
this.resultLabel.y = 1100;
this.resultLabel.depth = 100;


// this.gameoverScore = new PIXI.Text(''+score, ct.styles.get('Gameover Score')); 
// this.addChild(this.gameoverScore);
// this.gameoverScore.x = 900;
// this.gameoverScore.y = 1300;
// this.gameoverScore.depth = 100;
...
ct.rooms.templates['How To'] = {
    ...
    width: 2400,
    height: 2400,
    objects: JSON.parse('[{"x":1895.49147853,"y":266.87385787,"opacity":1,"tint":16777215,"scale":{"x":1.97567737,"y":1.97567737},"rotation":0,"exts":{},"customProperties":{},"template":"X"},{"x":1204.46679688,"y":1346.1055345,"opacity":1,"tint":16777215,"scale":{"x":2.48616536,"y":2.80853501},"rotation":0,"exts":{},"customProperties":{},"template":"CombiStackHowTo"}]'),
    ...
    onCreate() {
        /* room How To — core_OnRoomStart (On room start event) */
{
this.alpha = 0;
...
ct.templates.templates["Main_Title_page"] = {
    ...
    onStep: function () {
        /* template Main_Title_page — core_OnStep (On frame start event) */
{
this.move();
}
...
ct.templates.templates["Logolized_Combi_stack_blocks"] = {
    ...
    onStep: function () {
        /* template Logolized_Combi_stack_blocks — core_OnStep (On frame start event) */
{
this.move();
}
...
ct.templates.templates["Button_2_purple"] = {
    ...
    onStep: function () {
        /* template Button_2_purple — core_OnStep (On frame start event) */
{
this.move();
}
...
ct.templates.templates["Close"] = {
    ...
    texture: "Close",
    onStep: function () {
        /* template Close — core_OnStep (On frame start event) */
{
this.move();
}
...
ct.templates.templates["HowTo"] = {
    ...
    onCreate: function () {
        /* template HowTo — core_OnPointerClick (OnPointerClick event) */
this.interactive = true;
this.on('pointertap', () => {
    

ct.rooms.append('How To'); // create a room, and set it to a parameter `pauseMenu`

ct.sound.spawn('click')

});
/* template HowTo — core_OnPointerEnter (OnPointerEnter event) */
this.interactive = true;
this.on('pointerover', () => {
...
/* template HowTo — core_OnPointerLeave (OnPointerLeave event) */
this.interactive = true;
this.on('pointerout', () => {
...
ct.templates.templates["StartGame"] = {
    ...
    onCreate: function () {
        /* template StartGame — core_OnPointerClick (OnPointerClick event) */
this.interactive = true;
this.on('pointertap', () => {
    
ct.transition.circleOut(750, 0x34b8fe).then(() => {
  return ct.rooms.switch('Game');
});

ct.sound.spawn('click')

});
/* template StartGame — core_OnPointerEnter (OnPointerEnter event) */
this.interactive = true;
this.on('pointerover', () => {
...
/* template StartGame — core_OnPointerLeave (OnPointerLeave event) */
this.interactive = true;
this.on('pointerout', () => {
...
ct.templates.templates["BG_box"] = {
    ...
    texture: "BG_box",
    onStep: function () {
        /* template BG_box — core_OnStep (On frame start event) */
{
this.move();
}
...
ct.templates.templates["btnPause"] = {
    ...
    onCreate: function () {
        /* template btnPause — core_OnPointerClick (OnPointerClick event) */
this.interactive = true;
this.on('pointertap', () => {
    
if (ct.rooms.list['Game paused'].length === 0) {
    ct.rooms.append('Game paused', {
        isUi: true
    });
    ct.pixiApp.ticker.speed = 0;
    }  
...
/* template btnPause — core_OnPointerEnter (OnPointerEnter event) */
this.interactive = true;
this.on('pointerover', () => {
...
/* template btnPause — core_OnPointerLeave (OnPointerLeave event) */
this.interactive = true;
this.on('pointerout', () => {
...
ct.templates.templates["btnMusicOn"] = {
    ...
    onCreate: function () {
        /* template btnMusicOn — core_OnPointerClick (OnPointerClick event) */
this.interactive = true;
this.on('pointertap', () => {
...
if(!this.pressed){
    this.pressed = true
    this.kill = true
    ct.templates.copy('btnMusicOff', this.x, this.y)
    ct.sound.pause('bgm')
    paused = true;
}
...
/* template btnMusicOn — core_OnPointerEnter (OnPointerEnter event) */
this.interactive = true;
this.on('pointerover', () => {
...
/* template btnMusicOn — core_OnPointerLeave (OnPointerLeave event) */
this.interactive = true;
this.on('pointerout', () => {
...
ct.templates.templates["btnMechanics1"] = {
    ...
    onCreate: function () {
        /* template btnMechanics1 — core_OnPointerClick (OnPointerClick event) */
this.interactive = true;
this.on('pointertap', () => {
    
if (ct.rooms.list['Game paused'].length === 0) {
    ct.rooms.append('How To', {
        isUi: true
    });
...
/* template btnMechanics1 — core_OnPointerEnter (OnPointerEnter event) */
this.interactive = true;
this.on('pointerover', () => {
...
/* template btnMechanics1 — core_OnPointerLeave (OnPointerLeave event) */
this.interactive = true;
this.on('pointerout', () => {
...
ct.templates.templates["Combi_FS_choco"] = {
    ...
    onStep: function () {
        /* template Combi_FS_choco — core_OnStep (On frame start event) */
{
if(this.clockwise)
{
    this.angle += .2;
...
else if(!this.clockwise)
{
    ...
    onCreate: function () {
        /* template Combi_FS_choco — core_OnCreate (On create event) */
{
this.clockwise
}
...
ct.templates.templates["Combi_FS_Cow"] = {
    ...
    onDraw: function () {
        /* template Combi_FS_Cow — core_OnDraw (On frame end event) */
{
if(this.clockwise)
{
    this.angle += .5;
...
else if(!this.clockwise)
{
    ...
    onCreate: function () {
        /* template Combi_FS_Cow — core_OnCreate (On create event) */
{
this.clockwise = true
}
...
ct.templates.templates["Reveal_yellow_rectangle"] = {
    ...
    onStep: function () {
        /* template Reveal_yellow_rectangle — core_OnStep (On frame start event) */
{
this.move();
}
...
ct.templates.templates["BCrack"] = {
    ...
    onStep: function () {
        /* template BCrack — core_OnStep (On frame start event) */
{
this.move();
}
...
ct.templates.templates["BFill"] = {
    ...
    onStep: function () {
        /* template BFill — core_OnStep (On frame start event) */
{
this.move();
}
...
ct.templates.templates["BWafer"] = {
    ...
    onStep: function () {
        /* template BWafer — core_OnStep (On frame start event) */
{
this.move();
}
...
ct.templates.templates["Score_box"] = {
    ...
    texture: "Score_box",
    onStep: function () {
        /* template Score_box — core_OnStep (On frame start event) */
{
this.move();
}
...
ct.templates.templates["btnContinue"] = {
    ...
    onCreate: function () {
        /* template btnContinue — core_OnPointerClick (OnPointerClick event) */
this.interactive = true;
this.on('pointertap', () => {
...
ct.templates.templates["btnClose"] = {
    ...
    onCreate: function () {
        /* template btnClose — core_OnPointerClick (OnPointerClick event) */
this.interactive = true;
this.on('pointertap', () => {
...
if (!this.pressed) {
    ...
    .then(() => {
        ...
        ct.rooms.remove(this.getRoom());
        ct.transition.circleOut(750).then(() => {
            return ct.rooms.switch('Main');
            });
    });
}
...
ct.templates.templates["btnNew"] = {
    ...
    onCreate: function () {
        /* template btnNew — core_OnPointerClick (OnPointerClick event) */
this.interactive = true;
this.on('pointertap', () => {
...
/* template btnNew — core_OnPointerEnter (OnPointerEnter event) */
this.interactive = true;
this.on('pointerover', () => {
...
/* template btnNew — core_OnPointerLeave (OnPointerLeave event) */
this.interactive = true;
this.on('pointerout', () => {
...
ct.templates.templates["BG_vector_element_1"] = {
    ...
    onStep: function () {
        /* template BG_vector_element_1 — core_OnStep (On frame start event) */
{
this.move();
}
...
ct.templates.templates["BG_vector_element_2"] = {
    ...
    onStep: function () {
        /* template BG_vector_element_2 — core_OnStep (On frame start event) */
{
this.move();
}
...
ct.templates.templates["Button_1_blue"] = {
    ...
    onCreate: function () {
        /* template Button_1_blue — core_OnPointerClick (OnPointerClick event) */
this.interactive = true;
this.on('pointertap', () => {
...
ct.templates.templates["Button_3_pink"] = {
    ...
    onStep: function () {
        /* template Button_3_pink — core_OnStep (On frame start event) */
{
this.move();
}
...
ct.templates.templates["GCombi"] = {
    ...
    onStep: function () {
        /* template GCombi — core_OnStep (On frame start event) */
{
this.move();
}
...
ct.templates.templates["BLACK"] = {
    ...
    texture: "Placeholder",
    onStep: function () {
        /* template BLACK — core_OnStep (On frame start event) */
{
this.move();
}
...
ct.templates.templates["Scorebox_with_elements_for_reference"] = {
    ...
    onStep: function () {
        /* template Scorebox_with_elements_for_reference — core_OnStep (On frame start event) */
{
this.move();
}
...
ct.templates.templates["Text_gameover"] = {
    ...
    onStep: function () {
        /* template Text_gameover — core_OnStep (On frame start event) */
{
this.move();
}
...
ct.templates.templates["Button_Stack_Again"] = {
    ...
    onCreate: function () {
        /* template Button_Stack_Again — core_OnPointerClick (OnPointerClick event) */
this.interactive = true;
this.on('pointertap', () => {
...
ct.templates.templates["btnHome"] = {
    ...
    onCreate: function () {
        /* template btnHome — core_OnPointerClick (OnPointerClick event) */
this.interactive = true;
this.on('pointertap', () => {
...
if (!this.pressed) {
    ...
    .then(() => {
        ...
        ct.rooms.remove(this.getRoom());
        ct.transition.circleOut(750, 0x8069bf).then(() => {
            return ct.rooms.switch('Main');
            });
    });
}
...
ct.templates.templates["Button_share"] = {
    ...
    onStep: function () {
        /* template Button_share — core_OnStep (On frame start event) */
{
this.move();
}
...
ct.templates.templates["X"] = {
    ...
    onCreate: function () {
        /* template X — core_OnPointerClick (OnPointerClick event) */
this.interactive = true;
this.on('pointertap', () => {
...
/* template X — core_OnPointerEnter (OnPointerEnter event) */
this.interactive = true;
this.on('pointerover', () => {
...
/* template X — core_OnPointerLeave (OnPointerLeave event) */
this.interactive = true;
this.on('pointerout', () => {
...
ct.templates.templates["Holder"] = {
    ...
    texture: "Holder",
    onStep: function () {
        /* template Holder — core_OnStep (On frame start event) */
{
this.move();
}
...
ct.templates.templates["blue"] = {
    ...
    onDraw: function () {
        /* template blue — core_OnDraw (On frame end event) */
{
if(this.matched)
{
    this.angle += 10;
    ...
    onCreate: function () {
        /* template blue — core_OnCreate (On create event) */
{
this.matched = false;
...
ct.templates.templates["green"] = {
    ...
    onDraw: function () {
        /* template green — core_OnDraw (On frame end event) */
{
if(this.matched)
{
    this.angle += 10;
    ...
    onCreate: function () {
        /* template green — core_OnCreate (On create event) */
{
this.matched = false;
...
ct.templates.templates["yellow"] = {
    ...
    onDraw: function () {
        /* template yellow — core_OnDraw (On frame end event) */
{
if(this.matched)
{
    this.angle += 10;
    ...
    onCreate: function () {
        /* template yellow — core_OnCreate (On create event) */
{
this.matched = false;
...
ct.templates.templates["YCrack"] = {
    ...
    onStep: function () {
        /* template YCrack — core_OnStep (On frame start event) */
{
this.move();
}
...
ct.templates.templates["YFill"] = {
    ...
    onStep: function () {
        /* template YFill — core_OnStep (On frame start event) */
{
this.move();
}
...
ct.templates.templates["YWafer"] = {
    ...
    onStep: function () {
        /* template YWafer — core_OnStep (On frame start event) */
{
this.move();
}
...
ct.templates.templates["Star"] = {
    ...
    onDraw: function () {
        /* template Star — core_OnDraw (On frame end event) */
{
if(this.clockwise)
{
    this.angle += 5;
...
ct.templates.templates["BG"] = {
    ...
    texture: "BG",
    onStep: function () {
        /* template BG — core_OnStep (On frame start event) */
{
this.move();
}
...
ct.templates.templates["btnMusicOff"] = {
    ...
    onCreate: function () {
        /* template btnMusicOff — core_OnPointerClick (OnPointerClick event) */
this.interactive = true;
this.on('pointertap', () => {
...
if(!this.pressed){
    this.pressed = true
    this.kill = true
    ct.templates.copy('btnMusicOn', this.x, this.y)
    ct.sound.resume('bgm')
    paused = false;
}

});
/* template btnMusicOff — core_OnPointerEnter (OnPointerEnter event) */
this.interactive = true;
this.on('pointerover', () => {
...
/* template btnMusicOff — core_OnPointerLeave (OnPointerLeave event) */
this.interactive = true;
this.on('pointerout', () => {
...
ct.templates.templates["btnResume"] = {
    ...
    onCreate: function () {
        /* template btnResume — core_OnPointerClick (OnPointerClick event) */
this.interactive = true;
this.on('pointertap', () => {
...
/* template btnResume — core_OnPointerEnter (OnPointerEnter event) */
this.interactive = true;
this.on('pointerover', () => {
...
/* template btnResume — core_OnPointerLeave (OnPointerLeave event) */
this.interactive = true;
this.on('pointerout', () => {
...
ct.templates.templates["btnMechanics2"] = {
    ...
    onCreate: function () {
        /* template btnMechanics2 — core_OnPointerClick (OnPointerClick event) */
this.interactive = true;
this.on('pointertap', () => {
...
if(!this.pressed){
    this.pressed = true
    this.kill = true
    ct.templates.copy('btnMechanics1', this.x, this.y)
}
...
ct.templates.templates["btnCont1"] = {
    ...
    onCreate: function () {
        /* template btnCont1 — core_OnPointerClick (OnPointerClick event) */
this.interactive = true;
this.on('pointertap', () => {
...
/* template btnCont1 — core_OnPointerEnter (OnPointerEnter event) */
this.interactive = true;
this.on('pointerover', () => {
...
/* template btnCont1 — core_OnPointerLeave (OnPointerLeave event) */
this.interactive = true;
this.on('pointerout', () => {
...
ct.templates.templates["NewGame"] = {
    ...
    onCreate: function () {
        /* template NewGame — core_OnPointerClick (OnPointerClick event) */
this.interactive = true;
this.on('pointertap', () => {
...
ct.templates.templates["StackAgain"] = {
    ...
    onCreate: function () {
        /* template StackAgain — core_OnPointerClick (OnPointerClick event) */
this.interactive = true;
this.on('pointertap', () => {
...
/* template StackAgain — core_OnPointerEnter (OnPointerEnter event) */
this.interactive = true;
this.on('pointerover', () => {
...
/* template StackAgain — core_OnPointerLeave (OnPointerLeave event) */
this.interactive = true;
this.on('pointerout', () => {
...
ct.templates.templates["btnHome2"] = {
    ...
    onCreate: function () {
        /* template btnHome2 — core_OnPointerLeave (OnPointerLeave event) */
this.interactive = true;
this.on('pointerout', () => {
...
this.kill = true
ct.templates.copy('btnHome', this.x, this.y)
...
ct.templates.templates["btnContinue2"] = {
    ...
    onCreate: function () {
        /* template btnContinue2 — core_OnPointerClick (OnPointerClick event) */
this.interactive = true;
this.on('pointertap', () => {
...
/* template btnContinue2 — core_OnPointerEnter (OnPointerEnter event) */
this.interactive = true;
this.on('pointerover', () => {
...
/* template btnContinue2 — core_OnPointerLeave (OnPointerLeave event) */
this.interactive = true;
this.on('pointerout', () => {
...
ct.templates.templates["btnContinue1"] = {
    ...
    onCreate: function () {
        /* template btnContinue1 — core_OnPointerEnter (OnPointerEnter event) */
this.interactive = true;
this.on('pointerover', () => {
...
this.kill = true
ct.templates.copy('btnContinue2', this.x, this.y)
...
ct.templates.templates["CombiStackHowTo"] = {
    ...
    onStep: function () {
        /* template CombiStackHowTo — core_OnStep (On frame start event) */
{
this.move();
}
    ...
    onCreate: function () {
        /* template CombiStackHowTo — core_OnCreate (On create event) */
{
...
ct.templates.templates["Progress"] = {
    ...
    texture: "Progress",
    onStep: function () {
        /* template Progress — core_OnStep (On frame start event) */
{
...
(function resAddon(ct) {
    ...
    ct.res = {
        ...
        textures: {},
        skeletons: {},
        groups: [{"fonts":{"ungrouped":["BoldenVan","BalooChettan-Regular","wonderbar.regular"]},"textures":{"ungrouped":["Placeholder","Bar","BG","Button_1_blues","btnCont","NewGame","StackAgain","btnContinue2","btnContinue1","CombiStackHowTo","ProgressHolder","Progress"],"Blocks":["Block_yellow_ctcheese","Block_green_combi","Block_blue_ctc"],"Background":["BG_Main","BG_box","BG_vector_element_1","BG_vector_element_2","Combi_FS_Cow","Combi_FS_choco","Holder"],"Button Mechanics":["Button_music","Button_music_change_color","Button_pause","Button_mechanics_change_color","Button_mechanics","Button_pause_change_color"],"Blue Block Reveal":["Reveal_crackers","Reveal_fillings","Reveal_wafers"],"Green Block Reveal":["Reveal_combi"],"Yellow Block Reveal":["Reveal_yellow_rectangle","Reveal_filling","Reveal_cracker","Reveal_wafer"],"Score":["Score","Score_box"],"Buttons":["Button_1_blue","Button_continue_change_color","Button_close_change_color","Button_New_game_change_color","Button_New_game","Button_close","Button_2_purple","Button_home","Button_home_change_color","Button_share","Button_share_change_color","Button_Stack_Again","Button_3_pink","HowTo","Close","StartGame","X"],"Text & Others":["Text_gameover","Text_total_score","Scorebox","Cheese","Logolized_Combi_stack_blocks","Splash_yellow","Scorebox_with_elements_for_reference","Splash_chocolate","Main_Title_page","How_to_play","Star","Progress_bar"]},"styles":{"ungrouped":["MainLabel","CenterLabel","ScoreLabel","LevelLabel","Gameover Score"]},"rooms":{"ungrouped":["Main","Game","Game paused","Game over","How To"]},"templates":{"ungrouped":["BG","btnContinue2","btnContinue1","CombiStackHowTo","ProgressHolder","Progress"],"Text & BG":["Main_Title_page","Logolized_Combi_stack_blocks","BG_box","Combi_FS_choco","Combi_FS_Cow","Reveal_yellow_rectangle","BG_vector_element_1","BG_vector_element_2","BLACK","Scorebox_with_elements_for_reference","Text_gameover","Button_Stack_Again","Holder","Star"],"Buttons":["Button_2_purple","Close","HowTo","StartGame","How_to_play","btnPause","btnMusicOn","btnMechanics1","btnContinue","btnClose","btnNew","Button_1_blue","Button_3_pink","btnHome","Button_share","X","btnMusicOff","btnResume","btnMechanics2","btnCont1","NewGame","StackAgain","btnHome2"],"Box":["BCrack","BFill","BWafer","GCombi","YCrack","YFill","YWafer"],"Score":["Score","Score_box"],"Blocks":["blue","green","yellow"]},"sounds":{"ungrouped":["8_bit","bloop","battle_sound","sweep_notifs","whoop","button","Sound_bQ7Lk2","bgm","pop","levelup","Sound_kFw56H","click","gameover"]},"emitterTandems":{"ungrouped":[]}}][0],
        /**
        ...
        loadGame() {
            ...
            const atlases = [["./img/a0.json","./img/a1.json","./img/a2.json","./img/a3.json","./img/a4.json","./img/a5.json","./img/a6.json","./img/a7.json","./img/a8.json","./img/a9.json","./img/a10.json","./img/a11.json","./img/a12.json","./img/a13.json"]][0];
            const tiledImages = [{}][0];
            const sounds = [[{"name":"8_bit","wav":false,"mp3":"./snd/cFHCtGQ39pd9Gr.mp3","ogg":false,"poolSize":5,"isMusic":false},{"name":"bloop","wav":false,"mp3":"./snd/pp6nHChbhTH47b.mp3","ogg":false,"poolSize":5,"isMusic":false},{"name":"battle_sound","wav":false,"mp3":"./snd/4NBRpBpM4zzdHg.mp3","ogg":false,"poolSize":5,"isMusic":false},{"name":"sweep_notifs","wav":false,"mp3":"./snd/hbR2FWwb2Hm6Bp.mp3","ogg":false,"poolSize":5,"isMusic":false},{"name":"whoop","wav":false,"mp3":"./snd/ctJ2zrrL8nC9fM.mp3","ogg":false,"poolSize":5,"isMusic":false},{"name":"button","wav":false,"mp3":"./snd/gpddcw7jMm7h5f.mp3","ogg":false,"poolSize":5,"isMusic":false},{"name":"Sound_bQ7Lk2","wav":false,"mp3":"./snd/PzHBqrqKbQ7Lk2.mp3","ogg":false,"poolSize":5,"isMusic":false},{"name":"bgm","wav":false,"mp3":"./snd/dgwMFCBgTMFTCf.mp3","ogg":false,"poolSize":5,"isMusic":true},{"name":"pop","wav":false,"mp3":"./snd/3cDq469g3dDgf7.mp3","ogg":false,"poolSize":5,"isMusic":false},{"name":"levelup","wav":false,"mp3":"./snd/gDQz5qMQQrdB4m.mp3","ogg":false,"poolSize":5,"isMusic":false},{"name":"Sound_kFw56H","wav":false,"mp3":"./snd/WRMjKFQCkFw56H.mp3","ogg":false,"poolSize":5,"isMusic":false},{"name":"click","wav":false,"mp3":"./snd/n4dD66c22F4J3L.mp3","ogg":false,"poolSize":5,"isMusic":false},{"name":"gameover","wav":false,"mp3":"./snd/Hw2bpD7gzLb95g.mp3","ogg":false,"poolSize":5,"isMusic":false}]][0];
            const bitmapFonts = [{}][0];
...
var RearrangeGrid = function(room, mType)
{
    if(!room.rearranging)
    {
        room.rearranging = true;
        if(ct.camera.shake <= 0)
            ct.camera.shake += 0.1
            
        if(room.preview[0] != null)
        {
            for (let i = 0; i< room.preview.length;i++)
                room.preview[i].kill = true;
        ...
        switch(mType)
        {
            case 1:
            //blue
            room.preview[0] = ct.templates.copy('BCrack',1792,1472);
            room.preview[1] = ct.templates.copy('BFill',1984,1472);
            room.preview[2] = ct.templates.copy('BWafer',2176,1472);
            break;

            case 2:
            //green
            room.preview[0] = ct.templates.copy('GCombi',1792,1472);
            room.preview[1] = ct.templates.copy('GCombi',1984,1472);
            room.preview[2] = ct.templates.copy('GCombi',2176,1472);
            break

            case 3:
            //yellow
            room.preview[0] = ct.templates.copy('YCrack',1792,1472);
            room.preview[1] = ct.templates.copy('YFill',1984,1472);
            room.preview[2] = ct.templates.copy('YWafer',2176,1472);
            break;
        }
        ...
        if(room.preview[0] != null)
        {
            for(let i = 0;i < room.preview.length;i++)
            {
                room.preview[i].scale.x = 0.5;
                room.preview[i].scale.y = 0.5;
            }
        }
        ...
        for (let x = 0; x < room.grid[0].length; x++) {
            let emptySpaces = 0;
            for (let y = room.grid.length - 1; y >= 0; y--) {
                if (room.grid[y][x] === 0) {
                emptySpaces++;
                } else if (emptySpaces > 0) {
                    room.grid[y + emptySpaces][x] = room.grid[y][x];
                    room.gridBlocks[y + emptySpaces][x] = room.gridBlocks[y][x];
                    room.gridBlocks[y][x].y += emptySpaces * 256;
                    room.grid[y][x] = 0;
                    room.gridBlocks[y][x] = null;
                }
            }
        }
        
        room.rearranging = false;
        CheckPattern(room);
    }
}

var CheckPattern = function(room)
{
    matchFound = false;
    ...
    if (!matchFound)
    {
        for (let x = 0; x < room.grid[0].length; x++) {
            for (let y = 0; y < room.grid.length; y++) {
                if(room.grid[y][x] > 0)
                {
                    if (room.grid[y][x] === room.grid[y][x+ 1] && room.grid[y][x] === room.grid[y][x + 2]) {
                        // console.log('Found match horizontal: ');
                        matchType = room.grid[y][x];
                        room.grid[y][x] = 0; room.grid[y][x+1] = 0;room.grid[y][x+2]=0;
                        room.gridBlocks[y][x].matched = true;room.gridBlocks[y][x+1].matched = true;room.gridBlocks[y][x+2].matched = true;
                        room.gridBlocks[y][x] = null;room.gridBlocks[y][x+1] = null;room.gridBlocks[y][x+2] = null;
                        score+=10;
                        if (!paused) ct.sound.spawn('pop')
                        room.scoreLabel.text = score;

                        tempScore += 10;
                        room.progressBar.scale.x = ((tempScore)/(60))

                        if(score % 60 == 0)
                        {
                            room.level += 1;
                            room.levelLabel.text = 'Level '+room.level;
                            room.dropTime -= (room.dropTime > .025) ? .025 : 0;
                            if (!paused) ct.sound.spawn('levelup')
                            room.progressBar.scale.x = 0;
                            tempScore=0;
                        }
                        matchFound = true;
                    }
    ...
    if (!matchFound)
    {
        ...
        for (let x = 0; x < room.grid[0].length; x++) {
            for (let y = 0; y < room.grid.length-2; y++) {
                if(room.grid[y][x] > 0)
                {
                    if (room.grid[y][x] === room.grid[y+1][x] && room.grid[y][x] === room.grid[y+2][x]) {
                        // console.log('Found match vertical: ');
                        matchType = room.grid[y][x];
                        room.grid[y][x] = 0; room.grid[y+1][x] = 0;room.grid[y+2][x]=0;
                        room.gridBlocks[y][x].matched = true;room.gridBlocks[y+1][x].matched = true;room.gridBlocks[y+2][x].matched = true;
                        room.gridBlocks[y][x] = null;room.gridBlocks[y+1][x] = null;room.gridBlocks[y+2][x] = null;
                        score+=10;
                        if (!paused) ct.sound.spawn('pop')
                        room.scoreLabel.text = score;
    ...
    if (!matchFound)
    {
        ...
        for (let x = 0; x < room.grid[0].length-2; x++) {
            for (let y = 0; y < room.grid.length-2; y++) {
                if(room.grid[y][x] > 0)
                {
                    if (room.grid[y][x] === room.grid[y+1][x+1] && room.grid[y][x] === room.grid[y+2][x+2]) {
                        // console.log('Found match diagonal (top-left to bottom-right): ');
                        matchType = room.grid[y][x];
                        room.grid[y][x] = 0; room.grid[y+1][x+1] = 0;room.grid[y+2][x+2]=0;
                        room.gridBlocks[y][x].matched = true;room.gridBlocks[y+1][x+1].matched = true;room.gridBlocks[y+2][x+2].matched = true;
                        room.gridBlocks[y][x] = null;room.gridBlocks[y+1][x+1] = null;room.gridBlocks[y+2][x+2] = null;
                        score+=10;
                        if (!paused) ct.sound.spawn('pop')
                        room.scoreLabel.text = score;
    ...
    if(!matchFound)
    {
        ...
        for (let x = 2; x < room.grid[0].length; x++) {
            for (let y = 0; y < room.grid.length-2; y++) {
                if(room.grid[y][x] > 0)
                {
                    if (room.grid[y][x] === room.grid[y+1][x-1] && room.grid[y][x] === room.grid[y+2][x-2]) {
                        // console.log('Found match diagonal (top-right to bottom-left): ');
                        matchType = room.grid[y][x];
                        room.grid[y][x] = 0; room.grid[y+1][x-1] = 0;room.grid[y+2][x-2]=0;
                        room.gridBlocks[y][x].matched = true;room.gridBlocks[y+1][x-1].matched = true;room.gridBlocks[y+2][x-2].matched = true;
                        room.gridBlocks[y][x] = null;room.gridBlocks[y+1][x-1] = null;room.gridBlocks[y+2][x-2] = null;
    ...
    console.log(room.progressBar.scale.x);

    room.dropped = false;
    room.timer2 = room.dropTime;
};
